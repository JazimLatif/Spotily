...
CREATE TABLE playlists (id SERIAL PRIMARY KEY, playlist_user INT REFERENCES users(id));

CREATE TABLE playlist_songs (id SERIAL PRIMARY KEY, playlist_id INT REFERENCES playlist(id),
...
CREATE TABLE user_answers (id SERIAL PRIMARY KEY, user_id INT REFERENCES users(id),
 question_id INT REFERENCES question(id), choice_id INT REFERENCES options(id));